<template>
  <main class="home-container">
    ...
    <aside id="sidebar" aria-label="Map location information">
      ...
      {{ location.center.lat.toFixed(4) }} | Zoom: {{ location.zoom.toFixed(2) }}
      <button @click="resetLocation">Reset</button>
    </aside>

    <Map v-model="location" aria-label="Interactive map" />
  </main>
</template>
...
export default {
  ...
  data() {
    return {
      location: { center: { lng: 144.9631, lat: -37.8136 }, zoom: 12 },
    }
  },
  methods: {
    resetLocation() {
      this.location = { center: { lng: 144.9631, lat: -37.8136 }, zoom: 12 }
    },
  },
}
</script>
